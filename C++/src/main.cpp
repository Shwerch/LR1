...
int main(int argc, char* argv[]) {
    ...
    if (command[0] == 'M') {
        Array<int64_t> data;
        ArrayHelper<int64_t> data_h;
        data_h.load(filename, &data);

        if (std::strcmp(command, "MPUSH") == 0 && argc >= 4) {
            data.push_back(atoi(argv[3]));
        } else if (std::strcmp(command, "MPOP") == 0 && argc >= 4) {
            data.pop_back();
        } else if (std::strcmp(command, "MDEL") == 0 && argc >= 4) {
            data.remove(atoi(argv[3]));
        } else if (std::strcmp(command, "MGET") == 0 && argc >= 4) {
            std::cout << data.get(atoi(argv[3])) << std::endl;
        }
        data_h.save(filename, data);
    } else if (command[0] == 'S') {
        Stack<int64_t> data;
        StackHelper<int64_t> data_h;
        data_h.load(filename, &data);

        if (std::strcmp(command, "SPUSH") == 0 && argc >= 4) {
            data.push(atoi(argv[3]));
